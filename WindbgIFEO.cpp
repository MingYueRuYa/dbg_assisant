...
WindbgIFEO::WindbgIFEO(QWidget* parent)
  ...
  ui.setupUi(this);
  _workbranch_ptr = std::make_shared<std::threadpool>();
  this->_init_search_opt();
  this->_init_ui();
  this->_init_signal();
...
void WindbgIFEO::on_auto_start_stateChanged(int state) {
  ...
  if (Qt::Checked == state) {
    QString cur_path = "\"" + QDir::toNativeSeparators(qApp->applicationFilePath()) + "\"";
...
WindbgIFEO::map_qstring WindbgIFEO::_diff_windbg_path(map_qstring src, map_qstring& dst)
{
  map_qstring result;
  for (auto& item : src) {
...
bool WindbgIFEO::_load_windbg_path()
{
  vector<string> paths = this->_settings.get_dbg_path();
  if (paths.empty())
  {
    return false;
  }
  else {
    std::for_each(paths.begin(), paths.end(), [this](const string& path) {
      QString q_path = QString::fromStdString(path);
      if (this->_map_windbg_path.find(q_path) == this->_map_windbg_path.end()) {
        this->_map_windbg_path[q_path] = q_path;
      }
      });
  }
  this->log_info(tr("Find windbg path cache, not search again"));
  return true;
}

void WindbgIFEO::_update_path_combo(const map_qstring& path)
{
  this->log_info(tr("Finished searching for windbg"));
  ...
  std::for_each(
    ...
    [this](const std::pair<QString, QString>& value) {
      ...
      if (windbg_path.contains("arm")) {
        ...
        this->_settings.add_dbg_path(value.second.toStdString());
...
void WindbgIFEO::_init_search_opt()
{
  this->log_info(tr("Searching for windbg.exe or DbgX.Shell.exe, Please wait a moment"), LOG_TYPE::INFO);
  _search_helper = std::make_unique<NTFSSearchHelper>(this->_workbranch_ptr);
  auto cb = [this](std::vector<wstring> file_path) {
    QStringList paths;
    for (auto& item : file_path) {
      paths << QString::fromStdWString(item);
    }
    this->finished_windbg_exes_ex(paths);
  };
  _search_helper->set_finish_call_back(cb);
  _search_helper->set_search_file_name({ L"DbgX.Shell.exe", L"windbg.exe" });

  if (this->_load_windbg_path())
  {
    //TODO 已经缓存了windbgpath
    this->_update_path_combo(this->_map_windbg_path);
  }
  else {
    _search_helper->start();
  }
...
void WindbgIFEO::_init_comobo() {
  ...
  for (auto& item : vec_com) {
    ComboCompleter* com = new ComboCompleter(item->model(), this);
...
void WindbgIFEO::_init_signal() {
  ...
  connect(this, SIGNAL(finished_windbg_exes_ex(const QStringList&)), this,
    SLOT(on_update_ex_windbg_path(const QStringList&)), Qt::QueuedConnection);
}

void WindbgIFEO::on_update_ex_windbg_path(const QStringList& path) {
  if (path.empty()) {
    return;
  }
  map_qstring now_str = {};
  ...
  map_qstring dbg_path = this->_diff_windbg_path(now_str, this->_map_windbg_path);
  this->_update_path_combo(dbg_path);
}

void WindbgIFEO::on_update_windbg_path() {
  this->log_info(tr("finished search windbg path"));
  disconnect(ui.comboBox_windbg_path, SIGNAL(currentTextChanged(QString)), this,
    SLOT(on_comboBoxChanged(QString)));
...
{
  if (!this->_search_helper->start())
  {
    this->log_info(tr("It's already search"), LOG_TYPE::WARNING);
  }
}
