...
signals:
  ...
  void finished_windbg_exes_ex(const QStringList& path);
...
protected slots:
  ...
  void on_update_ex_windbg_path(const QStringList& path);
...
private:
  ...
  map_qstring _diff_windbg_path(map_qstring src, map_qstring& dst);
  bool _load_windbg_path();
  void _update_path_combo(const map_qstring& path);
...
private:
  ...
  std::unique_ptr<NTFSSearchHelper> _search_helper;
};
